...
Scenario: Test 8.1 on URL /.acl
  * def requestUri = testContainer.getUrl() + '.acl'
  ...
  And request '@prefix acl: <http://www.w3.org/ns/auth/acl#>. <#owner> a acl:Authorization;   acl:agent <https://alice.idp.test.solidproject.org/profile/card#me>;   acl:accessTo </test-auth-rs/>;   acl:mode acl:Read, acl:Write, acl:Control. <#bob> a acl:Authorization;   acl:agent <https://bobwebid.idp.test.solidproject.org/profile/card#me>;   acl:accessTo </test-auth-rs/>;   acl:mode acl:Read, acl:Write. '
  When method PUT
  Then status 201
