Feature: Check that Bob can read and append to RDF resource when he is authorized read-append.
    ...
    * def acl =
    """
      aclPrefix
       ...
       + createBobAccessToAuthorization(webIds.bob, resource.getUrl(), 'acl:Read, acl:Append')
    """
  ...
  Scenario: Test 5.6 Write resource (PATCH) with delete denied
    Given url requestUri
    And headers clients.bob.getAuthHeaders('PATCH', requestUri)
    And header Content-Type = 'application/sparql-update'
  ...
  Scenario: Test 5.7 Append resource (POST) denied
    Given url requestUri
